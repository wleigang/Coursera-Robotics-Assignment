...
% Drawing the map and Current pose. 
figure(10);
imagesc(map); 
hold on;
colormap('gray');
...
grid;
hold on; 
yActual = plot(t(1), pose(2,1),'k', 'LineWidth', 2);
...
grid;
hold on; 
thetaActual = plot(t(1), pose(3,1),'k', 'LineWidth', 2);
...
% You will start estimating myPose from j=2 using ranges(:,2).
for j = 2:N 
